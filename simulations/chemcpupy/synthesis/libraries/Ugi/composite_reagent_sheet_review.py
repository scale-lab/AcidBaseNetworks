...
@description: script to review a composite library sourceplatesheet and to optionally 
              ...
              Composite Library Style #1
              - each reaction has slots for 1xAmine, 1xAldehyde, 2xCarboxylicAcids, 2xIsocyanides 
              - all combinations (with replacement) are generated              
              - the order of inclusion is Amine, Aldehyde, CarboxylicAcid_1, Isocyanide_1, CarboxylicAcid_2, Isocyanide_2
              - the volume included (following the above order) is 200, 200, 100, 100, 100, 100 nL
              - the library is repeate 2x on a single MALDI 1,536 plate       
...
# (make all possible combinations of {1xAmine, 1xAldehyde, 2xCarboxylicAcid, 2xIsocyanide} and do so twice)        
...
count_car2 = min([len(x) for x in loc_car]);                 
                    ...
                    for n_car2 in range(0,num_car):
                        for n_iso2 in range(0,num_iso):      
                            ...
                            if counter_iso==count_iso: 
                                counter_iso=0;
                            pos_use_car2.append(reag['position'][ind_car][n_car2][counter_car2]); 
                            counter_car2=counter_car2+1; 
                            if counter_car2==count_car2: 
                                counter_car2=0;
                            pos_use_iso2.append(reag['position'][ind_iso][n_iso2][counter_iso]); 
...
reagent_count = len(reag['class'])+2;
reagent_concentration = [0.5]*reagent_count; # [M]
reagent_transfer_volume = [200,200,100,100,100,100]; #[200,200,200,160]; #[100,100,100,80]; #[100]*reagent_count; # [nL]                      
reagent_pos = [ pos_use_ami, pos_use_ald, pos_use_car,  pos_use_iso, pos_use_car2,  pos_use_iso2 ];
reagent_class = [ 'amine',  'aldehyde', 'carboxylic acid', 'isocyanide', 'carboxylic acid', 'isocyanide' ];  
reagent_type = [ 'amine 1',  'aldehyde 1', 'carboxylic acid 1', 'isocyanide 1', 'carboxylic acid 2', 'isocyanide 2' ];  
reagent_symbol = [ 'Ami1',  'Ald1', 'Car1', 'Iso1', 'Car2', 'Iso2' ];  
