...
# Demo information
demo_name = 'acidbase_3bit_16x16_demo'
demo_file = 'demo_export.mat'
demo_source_platesheet = 'demo_source_platesheet.csv'
data_dir = os.path.join(path_script, 'data')
kernel_file = os.path.join(data_dir, 'kernel_mem_3bit_256.txt')
# img_name = 'img_zero_71'
img_name = 'img_one_91'
sample_image1 = os.path.join(data_dir, '3bit_images_256/' + img_name + '.txt')
...
vol1536_data = 1500 # volume to transfer from source to data plate [nL]
...
network.load_image(sample_image1, img_levels=[-1, 1, 3, 5])
...
pos_list_data_source, pos_list_data_destin, water_pos_list_data_source, water_pos_list_data_destin = network.generate_weighted_data_plate(
...
task = chemcpupy.TransferTask(from_plate=source_plate,
                        from_positions=pos_list_data_source + pos_list_indicator_source + water_pos_list_data_source,
                        to_plate=data_plate,
                        to_positions=pos_list_data_destin + pos_list_indicator_destin + water_pos_list_data_destin,
                        transfer_volumes=[vol1536_data] * (len(pos_list_data_destin) + len(water_pos_list_data_destin)) + [indicator_transfer_volume] * len(pos_list_indicator_destin),
