...
% grab file info
positions = h5read(filename,'/raw/position');
Npos = length(positions);
index = 1; 
position = positions{index};
sTime = '/raw/time';
sSignal = '/raw/signal';
hTime = h5info(filename,sTime);
hSignal = h5info(filename,sSignal);
hShape = hSignal.Dataspace.Size;
Npts = hShape(1);
time = h5read(filename,sTime,1,Npts);
...
figure('color','w','name','Time Series')
plot(time,signal)
xlabel('Time [s]')
ylabel('Signal')
box on
