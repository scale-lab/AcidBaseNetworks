...
# -*- coding: utf-8 -*-
"""
Created on Thur August 30 2018
...
@description: script to review a composite library sourceplatesheet and to optionally 
              ...
              - the volume included (following the above order) is 100, 100, 100, 100,... nL
    ...
    for n_ami in range(0,num_ami):
        for n_ald in range(0,num_ald):
            for n_car in range(0,num_car):
                for n_iso in [0]:
                    for n_ami2 in range(0,num_ami):
                        for n_ald2 in range(0,num_ald):
                            for n_car2 in range(0,num_car):
                                for n_iso2 in [1]:
                                    for n_ami3 in range(0,num_ami):
                                        for n_ald3 in range(0,num_ald):
                                            for n_car3 in range(0,num_car):
                                                for n_iso3 in [2]:                    
                                                    pos_use_ami[0].append(reag['position'][ind_ami][n_ami][counter_ami]); 
                                                    pos_use_ami[1].append(reag['position'][ind_ami][n_ami2][counter_ami]); 
                                                    pos_use_ami[2].append(reag['position'][ind_ami][n_ami3][counter_ami]); 
                                                    counter_ami = set_counter(counter_ami,count_ami);
                                                    
                                                    pos_use_ald[0].append(reag['position'][ind_ald][n_ald][counter_ald]); 
                                                    pos_use_ald[1].append(reag['position'][ind_ald][n_ald2][counter_ald]); 
                                                    pos_use_ald[2].append(reag['position'][ind_ald][n_ald3][counter_ald]); 
                                                    counter_ald = set_counter(counter_ald,count_ald);

                                                    pos_use_car[0].append(reag['position'][ind_car][n_car][counter_car]); 
                                                    pos_use_car[1].append(reag['position'][ind_car][n_car2][counter_car]); 
                                                    pos_use_car[2].append(reag['position'][ind_car][n_car3][counter_car]); 
                                                    counter_car = set_counter(counter_car,count_car);

                                                    pos_use_iso[0].append(reag['position'][ind_iso][n_iso][counter_iso]); 
                                                    pos_use_iso[1].append(reag['position'][ind_iso][n_iso2][counter_iso]); 
                                                    pos_use_iso[2].append(reag['position'][ind_iso][n_iso3][counter_iso]); 
