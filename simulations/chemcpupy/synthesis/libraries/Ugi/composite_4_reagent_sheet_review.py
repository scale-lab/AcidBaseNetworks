...
#source_plate_sheet = 'library_11';  # Composite Library 4 (Experiment 0161)
source_plate_sheet = 'library_11b'; # Composite Library 4b (Experiment 0162)
...
for n_rep in range(0,num_copies):   
    for n_seq in range(0,num_sequences):
        for n_pos in range(0,num_wells_to_use):
            ...
            if source_plate_sheet == 'library_11':
                n_iso = numpy.random.choice(range(0,num_iso))            
...
reagent_transfer_volume = [100]*reagent_count; # [100]*reagent_count; # [nL]                      
reagent_pos = flatten([[ pos_use_ami[n], pos_use_ald[n], pos_use_car[n],  pos_use_iso[n]] for n in range(0,num_sequences)]);
reagent_class = flatten([[ 'amine', 'aldehyde', 'carboxylic acid', 'isocyanide'] for n in range(1,num_sequences+1)]);
...
if save_variables:
    ...
    used_options = {'num_copies':num_copies, 'sort_transfers':sort_transfers, 'has_alignment_spots':add_alignment_spots, 'random_seed':random_seed, 'dilute_reaction':dilute_reaction};
