...
@author: chrisarcadia
@description: script to review a library sourceplatesheet and to optionally 
...
source_plate_sheet = 'library_6b2';
...
#for p_ami in pos_ami:
#    for p_ald in pos_ald:
#        for p_car in pos_car:
#            for p_iso in pos_iso:
#                pos_use_ami.append(p_ami);
#                pos_use_ald.append(p_ald);
#                pos_use_car.append(p_car);
#                pos_use_iso.append(p_iso);
#                pos_used.append(pos_empty.pop(0)); 
def choose_from_associated_well(loc,well_count,well_counter):
   if well_counter==well_count:
       well_counter=0;
   pos = loc[well_counter];   
   well_counter = well_counter + 1;
   return pos,well_counter;
well_counter_ami = [0]*num_ami;
well_counter_ald = [0]*num_ald;
well_counter_car = [0]*num_car;
well_counter_iso = [0]*num_iso;
for n_ami in range(0,num_ami):
    for n_ald in range(0,num_ald):
        for n_car in range(0,num_car):
            for n_iso in range(0,num_iso):
                # get the id's of each compound type to use                                
                p_ami,well_counter_ami[n_ami] = choose_from_associated_well(loc_ami[n_ami],well_count_ami[n_ami],well_counter_ami[n_ami]);
                p_ald,well_counter_ald[n_ald] = choose_from_associated_well(loc_ald[n_ald],well_count_ald[n_ald],well_counter_ald[n_ald]);
                p_car,well_counter_car[n_car] = choose_from_associated_well(loc_car[n_car],well_count_car[n_car],well_counter_car[n_car]);
                p_iso,well_counter_iso[n_iso] = choose_from_associated_well(loc_iso[n_iso],well_count_iso[n_iso],well_counter_iso[n_iso]);                
                pos_use_ami.append(p_ami);
                pos_use_ald.append(p_ald);
                pos_use_car.append(p_car);
                pos_use_iso.append(p_iso);                
...
# Transfer, in order of type, reagents from source to destination
sort_transfers = True;                            
reagent_type = [     'amine',  'aldehyde', 'carboxylic acid', 'isocyanide' ];  
reagent_class = [    'amine',  'aldehyde', 'carboxylic acid', 'isocyanide' ];  
reagent_symbol = [   'Ami',  'Ald', 'Car', 'Iso' ];  
reagent_count = len(reagent_type);
reagent_concentration = [0.5]*reagent_count; # [M]
reagent_transfer_volume = [200,200,200,160]; #[100,100,100,80]; #[100]*reagent_count; # [nL]  
reagent_pos = [ pos_use_ami, pos_use_ald,       pos_use_car,  pos_use_iso ];
reagent_num = [num_ami, num_ald, num_car, num_iso];
...
product_concentration = min(numpy.multiply(reagent_concentration,reagent_transfer_volume))/product_volume;
used_positions = sorted(list(set(pos_used)));
print('\n'+'Overview of the '+ str(reagent_count)+' reagent types to be used')
print(''+'(in the order in which they are added to the plate)')
print('  '+'| Type | Concentration | Transfer Volume | Variants | Equivalents |')
print('  '+'| --------------------------------------------------------------- | ')
for n in range(0,reagent_count):
    print('  '+'| '+reagent_class[n]+ ' | %0.1f mM' % (reagent_concentration[n]*1e3)+ ' | %0.1f nL' % (reagent_transfer_volume[n])+' | '+str(reagent_num[n]) +' | '+str(reagent_equivalence[n]) + ' |')
...
if save_variables:
    ...
    used_options = {'sort_transfers':sort_transfers, 'has_alignment_spots':add_alignment_spots};
