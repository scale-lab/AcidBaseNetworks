...
@description: script to review a composite library sourceplatesheet and to optionally 
              ...
              Composite Library Style #6 
              - each reaction has slots for N sequences of (1xAmine, 1xAldehyde, 1xCarboxylicAcids, 1xIsocyanides_n [cap]) 
              - all combinations (with replacement) are generated except for the isocyanides which occur once per sequence and symbolic and molecular caps/ends         
              - the order of inclusion is Amine, Aldehyde, CarboxylicAcid, Isocyanide xN
              - the volume included (following the above order) is 100, 100, 100, 100,... nL (substochiometric amounts for non-capping reagents)
              - the library is repeated 2x on a single MALDI 1,536 plate       
              - transfer volume is gradually increased so that reaction concentration is equal for each cycle
...
source_plate_sheet = 'library_13'; # Composite Library 6 (Experiment 0165)
...
source_working_volume = 40e3; # [nL] (for 384PP, when filled to 60uL)
print('\n'+'Loaded "' + filename + '"')
...
# (make all possible combinations of {1xAmine, 1xAldehyde, 1xCarboxylicAcid, 1xIsocyanide_n [cap]}x3 and do so twice)        
num_copies = 1; # number of copies
num_sequences = 8; #num_iso; # 5; # number of sequences (synthesis cycles)
delay_between_sequences = 1800e3; # [ms]
num_class = len(reag['class']); # number of classes
num_wells_to_use = len(pos_empty); # number of spots to make (wells to use)
pos_use_ami = [[] for n in range(0,num_sequences)]; # source
pos_use_ald = [[] for n in range(0,num_sequences)]; # source
pos_use_car = [[] for n in range(0,num_sequences)]; # source
pos_use_iso = [[] for n in range(0,num_sequences)]; # source
...
count_iso = [len(x) for x in loc_iso];  
def set_counter(count,limit):
    count=count+1; 
    if count==limit: 
        count=0;
    return count
...
if num_sequences > num_iso:
    raise ValueError('Not enough isocyanide species ('+ str(num_iso) +') to run '+ str(num_sequences) +' cycles.')
for n_rep in range(0,num_copies):   
    for n_seq in range(0,num_sequences):
        for n_pos in range(0,num_wells_to_use):

            n_ami = numpy.random.choice(range(0,num_ami))
            pos_use_ami[n_seq].append(reag['position'][ind_ami][n_ami][counter_ami]); 
            counter_ami = set_counter(counter_ami,count_ami);
            
            n_ald = numpy.random.choice(range(0,num_ald))            
            pos_use_ald[n_seq].append(reag['position'][ind_ald][n_ald][counter_ald]); 
            counter_ald = set_counter(counter_ald,count_ald);

            n_car = numpy.random.choice(range(0,num_car))            
            pos_use_car[n_seq].append(reag['position'][ind_car][n_car][counter_car]); 
            counter_car = set_counter(counter_car,count_car);

            n_iso = n_seq            
            pos_use_iso[n_seq].append(reag['position'][ind_iso][n_iso][counter_iso]); 
            counter_iso = set_counter(counter_iso,count_iso[n_iso]);
            
            if n_seq == 0:
                pos_used.append(pos_empty.pop(0));                         

# Generate the transfer volumes
volinit = 5e-9 # initial transfer volume [L]
concinit = 0.5 # initial reagent concentration [M]
voltx = numpy.zeros(num_sequences) # transfer volumes [L]
volcyc = numpy.zeros(num_sequences) # end of cycle volume [L]
conccyc = numpy.zeros(num_sequences) # end of cycle conc [M] 
concfin = numpy.zeros(num_sequences) # end of sequence conc [M]
method = 'matched'
for n in range(num_sequences):
    if n==0:
        ...
        if method == 'constant':
                alpha = 1                               
                voltx[n] = alpha*volinit # each cycles' transfers are the same volume      
        elif method == 'scaled':
                alpha = 2;
                voltx[n] = alpha*voltx[n-1] # each cycles' transfers are 2 times the previous
        elif 'matched':
                alpha = 1/num_class             
                voltx[n] = alpha*volcyc[n-1] # each cycles' transfers are set such that the volume added in cycle (n) matches the total volume after cycle (n-1)
        else:
                alpha = 1;                                                
                method = 'constant'
                voltx[n] = alpha*volinit # each cycles' transfers are the same volume      
    volcyc[n] = num_class*numpy.sum(voltx[0:(n+1)]) # total volume at then of cycle n
    conccyc[n] = concinit*voltx[n]/volcyc[n] # concentration of cycle n's reagents at the end of cycle n
for n in range(num_sequences):
    concfin[n] = conccyc[n]*(numpy.sum(voltx[0:(n+1)])/numpy.sum(voltx));

                        
# Transfer reagents from source to destination
sort_transfers = False;                            
reagent_count = num_class*num_sequences;
reagent_concentration = [concinit]*reagent_count; # [M]
reagent_transfer_volume = numpy.repeat(voltx,num_class)*1e9 # [60]*reagent_count; # [100]*reagent_count; # [nL]                      
reagent_pos = flatten([[ pos_use_ami[n], pos_use_ald[n], pos_use_car[n],  pos_use_iso[n]] for n in range(0,num_sequences)]);
reagent_delay = flatten([[ [delay_between_sequences*(n>0)]+[0]*(len(pos_use_ami[n])-1), [0]*len(pos_use_ald[n]), [0]*len(pos_use_car[n]),  [0]*len(pos_use_iso[n])] for n in range(0,num_sequences)]);reagent_class = flatten([[ 'amine', 'aldehyde', 'carboxylic acid', 'isocyanide'] for n in range(1,num_sequences+1)]);
reagent_type = flatten([[ 'amine '+str(n),  'aldehyde '+str(n), 'carboxylic acid '+str(n), 'isocyanide '+str(n)] for n in range(1,num_sequences+1)]);
reagent_symbol = flatten([[ 'Ami'+str(n),  'Ald'+str(n), 'Car'+str(n), 'Iso'+str(n)] for n in range(1,num_sequences+1)]);
...
tasklist_combo = chemcpupy.TaskList(description='Create Reagent Combinations');    
trans = {};
trans['source']=[];    
trans['destination']=[];    
trans['volume']=[];    
...
for n in range(0,reagent_count):
    ...
    task = chemcpupy.TransferTask(from_plate=plateSource,
                            ...
                            pre_transfer_delays=reagent_delay[n],
...
if dilute_reaction:
    ...
    solvent_transfer_volume = 1800;#3200; #1600; # [nL]
...
if save_variables:
    ...
    used_options = {'num_copies':num_copies, 'sort_transfers':sort_transfers, 'has_alignment_spots':add_alignment_spots, 'random_seed':random_seed, 'dilute_reaction':dilute_reaction, 'num_sequences':num_sequences, 'delay_between_sequences':delay_between_sequences};
    ...
    review_plates = {'source':plateSource.export_array_review(), 'destinationPreReaction':plateDestinInit.export_array_review()};    
    if simulate_reactions:
        review_plates.update({'destinationPostReaction':plateDestinRxn.export_array_review(), 'destinationUgiProducts':plateDestinRxn.export_array_review(typefilter='ugi'), 'MALDI':plateMALDI.export_array_review()});
